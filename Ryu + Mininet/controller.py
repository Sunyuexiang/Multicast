    ...
    def _monitor(self):
        ...
        while True:
            ...
            for folder in folders:
                rule_file =  os.path.join(path, folder, "file" + ".json")
                if not os.path.exists(rule_file):
                    continue
                rule = load_config(rule_file)
            ...
            # print "del group table:", list(set(self.pre_info) - set(temp))
            for item in list(set(self.pre_info) - set(temp)):
                print "del group id:", item
                print self.info
                index = item[-1]
                self.info.remove(item)
                # group_table.drop_group(index)
            self.pre_info = copy.deepcopy(temp)
            time.sleep(3)
